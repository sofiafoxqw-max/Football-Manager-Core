...
import { LayoutDashboard, Users, Building2, Store, DollarSign, Trophy, FileText, Activity } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  if (!isLoaded || (isSignedIn && meLoading)) {
    return <div className="min-h-screen flex items-center justify-center bg-background"><div className="animate-spin text-primary">...</div></div>;
  }

  // Hide sidebar on landing, auth, and onboarding
      ...
      <Sidebar className="w-64 border-r border-border hidden md:flex flex-col h-screen sticky top-0 bg-sidebar text-sidebar-foreground">
        ...
        <div className="p-4 border-t border-border mt-auto">
           <div className="text-xs text-muted-foreground font-mono flex items-center justify-between">
              <span>GFC Balance</span>
              <span className="text-primary font-bold">{me?.balance.toLocaleString()} GFC</span>
           </div>
        </div>
      </Sidebar>
